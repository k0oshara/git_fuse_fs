...
cleanup_mount(void)
{
  ...
  snprintf(cmd,
           sizeof(cmd),
           "fusermount3 -u %s 2>/dev/null || fusermount -u %s 2>/dev/null || "
           "umount %s 2>/dev/null",
           TEST_MOUNT_PATH,
           TEST_MOUNT_PATH,
           TEST_MOUNT_PATH);
...
mount_gitfs(void)
{
  ...
  if (gitfs_pid == 0) {
    // Child process
    execl(
      GITFS_BINARY, GITFS_BINARY, TEST_REPO_PATH, TEST_MOUNT_PATH, "-f", NULL);
    perror("execl gitfs");
    exit(1);
  } else if (gitfs_pid < 0) {
    perror("fork");
...
void
test_assert(const char* test_name, int condition)
{
  test_count++;
...
test_directory_listing(void)
{
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (strcmp(entry->d_name, "subdir") == 0)
      found_subdir = 1;
    if (strcmp(entry->d_name, "binary.dat") == 0)
      found_binary = 1;
  }
