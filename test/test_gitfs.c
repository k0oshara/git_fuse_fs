...
int mount_gitfs(void)
{
    ...
    {
        // Child process
        execl(GITFS_BINARY, GITFS_BINARY, TEST_REPO_PATH, TEST_MOUNT_PATH, "-f",
              NULL);
        perror("execl gitfs");
        exit(1);
    }
...
{
    struct stat st;
    test_assert("Directory exists", stat(TEST_MOUNT_PATH "/subdir", &st) == 0 &&
                                        S_ISDIR(st.st_mode));
}
...
void test_directory_listing(void)
{
    ...
    closedir(dir);
    test_assert("Directory listing",
                found_hello && found_subdir && found_binary);
}
