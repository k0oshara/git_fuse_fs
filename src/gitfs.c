...
static int
resolve_oid(const char* path, git_oid* oid_out, git_object_t* otype_out)
{
  struct path_cache_entry* pe;
  HASH_FIND_STR(path_cache, path, pe);
  if (pe) {
    fprintf(stderr,
            "[gitfs] path cache hit: %s → %s\n",
            path,
  ...
  if (strcmp(path, "/") == 0) {
    ...
    while (seg) {
      ...
      fprintf(stderr,
              "[gitfs]   segment=\"%s\" type=%s oid=%s\n",
              seg,
              otype == GIT_OBJECT_TREE ? "tree" : "blob",
              hex);
...
static int
load_tree(git_tree** out, const git_oid* oid)
{
  char hex[GIT_OID_HEXSZ + 1];
  git_oid_tostr(hex, sizeof(hex), oid);
...
static int
load_blob(git_blob** out, const git_oid* oid)
{
  char hex[GIT_OID_HEXSZ + 1];
  git_oid_tostr(hex, sizeof(hex), oid);
...
void*
gitfs_init(struct fuse_conn_info* conn, struct fuse_config* cfg)
{
  (void)conn;
  cfg->kernel_cache = 1;
  return GFS;
...
void
gitfs_destroy(void* private_data)
{
  free_cache();
...
int
gitfs_getattr(const char* path, struct stat* stbuf, struct fuse_file_info* fi)
{
  (void)fi;
...
int
gitfs_open(const char* path, struct fuse_file_info* fi)
{
  (void)fi;
  git_oid oid;
  git_object_t type;
